...
CCDeviceType_t getDeviceTypeFromString(char *deviceTypeStr) {
...
bool hasFeature(CCDeviceType_t deviceType, CCFeature_t feature) {
...
int listDevices(CCOptionValue_t options[]) {
    int fret, i;
...
struct ftdi_context* openDevice(CCOptionValue_t options[], CCDeviceType_t *deviceType) {
    struct ftdi_context *ftdi = NULL;
    int fret;
    char product[STRING_SIZE + 1];
    CCDeviceType_t tmpDeviceType;
...
int showInfo(CCOptionValue_t options[]) {
    struct ftdi_context *ftdi;
...
int doInit(CCOptionValue_t options[]) {
...
int setSerial(char *serialNumber, CCOptionValue_t options[]) {
    struct ftdi_context *ftdi;
...
struct ftdi_context* prepareDevice(CCOptionValue_t options[], unsigned char *pins, CCDeviceType_t *deviceType) {
    struct ftdi_context *ftdi;
    int f;
...
int setPins(unsigned char pins, CCOptionValue_t options[]) {
    CCDeviceType_t deviceType;
...
int showStatus(CCOptionValue_t options[]) {
	int ret = 0;
    unsigned char pins;
    CCDeviceType_t deviceType;
    struct ftdi_context *ftdi = prepareDevice(options, &pins, &deviceType);
...
int setDyPer(CCCommand_t cmd, CCOptionValue_t options[]) {
    unsigned char pins;
    bool switchOn;
    CCDeviceType_t deviceType;
...
#if 0
int parseArguments(int argc, const char **argv, CCCommand_t *cmd, int *arg, char *args, size_t argsLen,
                   CCOptionValue_t options[]) {
    int c;
    char *serial = NULL;
...
int main(int argc, const char **argv) {
    CCCommand_t cmd = CCC_None;
    int arg;
    char args[64];
    CCOptionValue_t options[CCO_MAX];
...
int external_selectTarget(Target_t target) {
    ...
    options[CCO_Vendor].argn = SAMSUNG_VENDOR;
    options[CCO_Product].argn = PRODUCT;
    options[CCO_DeviceId].argn = 0;

    if (selectTarget(target, options) != EXIT_SUCCESS) {
        return EXIT_FAILURE;
    }
